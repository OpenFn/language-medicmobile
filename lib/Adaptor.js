...
function changesApi(params) {
  ...
  return function (state) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      var nextState = _extends({}, state, { data: [].concat(_toConsumableArray(state.data), [data])
        // if (data.length) {
...
function pickFormData(formId) {
  ...
  return function (state) {
    ...
    var myFormData = state.data[0].results.filter(function (item) {
    ...
    var nextState = _extends({}, state, { data: [].concat(_toConsumableArray(state.data), [myFormData]) });
    return nextState;
  };
};
